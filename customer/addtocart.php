						...
						$insert = mysqli_query($koneksi, "INSERT INTO cart(id_cart, tanggal, kode, nama, harga, qty, jumlah, session)
                         ...
                         if($insert){
                          $qu			= mysqli_query($koneksi, "UPDATE produk SET stok=(stok-'$qty') WHERE kode='$kode'");
						         ...
						         echo "<script>alert('Produk ditambahkan ke keranjang!'); window.location = 'detail.php'</script>";   
                                    //}
                        }else{
						echo "<script>alert('Gagal Pesan!'); window.location = 'javascript:history.back()'</script>";
